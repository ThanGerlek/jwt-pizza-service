...
describe('user routes', () => {
  ...
  test('GET /api/user/me returns authenticated user', async () => {
    ...
    const res = await request(app).get('/api/user/me').set('Authorization', 'Bearer tok.sig.sgn');
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ id: 1, name: 'admin', email: 'a@jwt.com' });
  });
  ...
  test('PUT /api/user/:userId returns 403 for unauthorized user', async () => {
    ...
    jwt.verify = jest.fn(() => ({ id: 1, name: 'user', email: 'u@jwt.com', roles: [{ role: 'diner' }] }));
    const res = await request(app).put('/api/user/2').set('Authorization', 'Bearer tok.sig.sgn').send({ name: 'x' });
    expect(res.status).toBe(403);
  });

  test('PUT /api/user/:userId allows admin to update', async () => {
    DBModule.DB.isLoggedIn.mockResolvedValueOnce(true);
    DBModule.DB.updateUser.mockResolvedValueOnce({ id: 2, name: 'updated', email: 'u@test.com', roles: [{ role: 'diner' }] });

    // Make jwt.verify return an admin user for this request
    jwt.verify = jest.fn(() => ({ id: 2, name: 'admin', email: 'a@jwt.com', roles: [{ role: 'admin' }] }));

    const res = await request(app).put('/api/user/2').set('Authorization', 'Bearer tok.sig.sgn').send({ name: 'updated' });
    expect(res.status).toBe(200);
