...
jest.mock('jsonwebtoken', () => ({
  ...
  verify: jest.fn(() => ({ id: 2, name: 'pizza diner', email: 'x@test.com', roles: [{ role: 'diner' }] })),
}));
...
describe('auth routes', () => {
  ...
  test('register POST /api/auth returns user and token', async () => {
    const testUser = { name: 'pizza diner', email: 'r@test.com', password: 'pw' };
    DB.addUser.mockResolvedValueOnce({ id: 3, name: testUser.name, email: testUser.email, roles: [{ role: 'diner' }] });
    ...
    expect(res.body.user).toMatchObject({ id: 3, name: testUser.name, email: testUser.email, roles: [{ role: 'diner' }] });
    expect(res.body.user.password).toBeUndefined();
  });

  test('login PUT /api/auth returns user and token', async () => {
    const testUser = { email: 'l@test.com', password: 'pw' };
    DB.getUser.mockResolvedValueOnce({ id: 4, name: 'login', email: testUser.email, roles: [{ role: 'diner' }] });
    DB.loginUser.mockResolvedValueOnce();
  ...
  test('logout DELETE /api/auth requires auth and logs out', async () => {
    ...
    const res = await request(app).delete('/api/auth').set('Authorization', 'Bearer tok.sig.sgn');
    expect(res.status).toBe(200);
