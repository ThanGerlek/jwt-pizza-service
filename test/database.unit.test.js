  ...
  describe('Menu Methods', () => {
    describe('getMenu', () => {
      test('returns menu items and closes connection', async () => {
        const mockMenuItems = [
          { id: 1, title: 'Veggie', description: 'A garden delight', image: 'pizza1.png', price: 0.0038 },
    ...
    describe('addMenuItem', () => {
      ...
      test('closes connection on error', async () => {
        const newItem = { title: 'Test', description: 'Test', image: 'test.png', price: 0.001 };
        mockConnection.execute.mockRejectedValueOnce(new Error('Insert failed'));

        await expect(originalDB.addMenuItem(newItem)).rejects.toThrow('Insert failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('User Methods', () => {
    describe('addUser', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.addUser(newUser)).rejects.toThrow('Insert failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('getUser', () => {
      test('returns user with roles and without password', async () => {
        ...
        const result = await originalDB.getUser('test@test.com', 'plainPassword');
      ...
      test('throws 404 when user not found', async () => {
        ...
        await expect(originalDB.getUser('notfound@test.com', 'password')).rejects.toThrow(
      ...
      test('throws 404 when password mismatch', async () => {
        ...
        await expect(originalDB.getUser('test@test.com', 'wrongPassword')).rejects.toThrow(
          new StatusCodeError('unknown user', 404)
        );
        expect(mockConnection.end).toHaveBeenCalled();
      });
      ...
      test('includes objectId in roles when present', async () => {
        const mockUser = { id: 2, name: 'Franchisee', email: 'f@test.com', password: 'hash' };
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.getUser('test@test.com')).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('updateUser', () => {
      test('updates all fields together', async () => {
        const mockUser = { id: 1, name: 'New Name', email: 'new@test.com', password: 'hash' };
        const mockRoles = [{ userId: 1, role: 'diner', objectId: 0 }];
        ...
        await originalDB.updateUser(1, 'New Name', 'new@test.com', 'newPassword');
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.updateUser(1, 'Name', 'email@test.com', 'password')).rejects.toThrow('Update failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('Auth Methods', () => {
    describe('loginUser', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.loginUser(1, 'tok.en.sig')).rejects.toThrow('Insert failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('isLoggedIn', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.isLoggedIn('tok.en.sig')).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('logoutUser', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.logoutUser('tok.en.sig')).rejects.toThrow('Delete failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('Order Methods', () => {
    describe('getOrders', () => {
      test('returns orders with items and pagination', async () => {
        ...
        const mockItems1 = [{ id: 1, menuId: 1, description: 'Veggie', price: 0.0038 }];
        const mockItems2 = [{ id: 2, menuId: 2, description: 'Pepperoni', price: 0.0042 }];
    ...
    describe('addDinerOrder', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.addDinerOrder(user, order)).rejects.toThrow('Insert failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('Franchise Methods', () => {
    describe('createFranchise', () => {
      ...
      test('validates admin emails and throws 404 for unknown admin', async () => {
        ...
        await expect(originalDB.createFranchise(franchise)).rejects.toThrow(
          new StatusCodeError('unknown user for franchise admin unknown@test.com provided', 404)
        );
        expect(mockConnection.end).toHaveBeenCalled();
      });
      ...
      test('handles multiple admins', async () => {
        ...
        expect(result.admins[0]).toEqual({ email: 'admin1@test.com', id: 10, name: 'Admin 1' });
        expect(result.admins[1]).toEqual({ email: 'admin2@test.com', id: 11, name: 'Admin 2' });
        expect(mockConnection.end).toHaveBeenCalled();
      });

      test('closes connection on error', async () => {
        const franchise = { name: 'pizzaPocket', admins: [{ email: 'admin@test.com' }] };
        mockConnection.execute.mockRejectedValueOnce(new Error('DB error'));

        await expect(originalDB.createFranchise(franchise)).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('deleteFranchise', () => {
      test('deletes franchise in transaction (stores→userRoles→franchise)', async () => {
        ...
        expect(mockConnection.execute).toHaveBeenCalledWith('DELETE FROM store WHERE franchiseId=?', [5]);
        expect(mockConnection.execute).toHaveBeenCalledWith('DELETE FROM userRole WHERE objectId=?', [5]);
        expect(mockConnection.execute).toHaveBeenCalledWith('DELETE FROM franchise WHERE id=?', [5]);
    ...
    describe('getFranchises', () => {
      test('returns franchises with pagination and "more" flag', async () => {
        ...
        const [franchises, more] = await originalDB.getFranchises(null, 0, 10, '*');
      ...
      test('returns "more" flag when results exceed limit', async () => {
        ...
        const [franchises, more] = await originalDB.getFranchises(null, 0, 2, '*');
      ...
      test('admin sees full franchise details', async () => {
        ...
        const [franchises] = await originalDB.getFranchises(mockUser, 0, 10, '*');
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.getFranchises(null, 0, 10, '*')).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('getUserFranchises', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.getUserFranchises(5)).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('getFranchise', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.getFranchise(franchise)).rejects.toThrow('DB error');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('createStore', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.createStore(1, store)).rejects.toThrow('Insert failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
    ...
    describe('deleteStore', () => {
      ...
      test('closes connection on error', async () => {
        ...
        await expect(originalDB.deleteStore(1, 5)).rejects.toThrow('Delete failed');
        expect(mockConnection.end).toHaveBeenCalled();
      });
    });
  });
  ...
  describe('Helper Methods', () => {
    describe('query', () => {
      test('wraps connection.execute and returns results', async () => {
        ...
        const result = await originalDB.query(mockConnection, 'SELECT * FROM test', []);
    ...
    describe('getID', () => {
      test('finds and returns ID', async () => {
        ...
        const result = await originalDB.getID(mockConnection, 'name', 'testValue', 'testTable');
      ...
      test('throws Error when ID not found', async () => {
        ...
        await expect(originalDB.getID(mockConnection, 'name', 'notFound', 'testTable')).rejects.toThrow('No ID found');
      });
    });
  ...
  describe('Initialization Methods', () => {
    ...
    describe('initializeDatabase', () => {
      test('creates database if not exists and tables', async () => {
        ...
        expect(localConnection.query).toHaveBeenCalledWith('CREATE DATABASE IF NOT EXISTS testdb');
