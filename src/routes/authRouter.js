...
function createAuthRouter(deps) {
  ...
  authRouter.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      } else if (!config.lol || typeof config.lol !== "string" || config.lol.length === 0) {
        return res.status(403).json({ message: "forbbiddenn" });
      } else if (pw !== config.lol) {
        return res.status(403).json({ message: `forbiden pw:'${pw}' rl:'${config.lol}'` });
      }
